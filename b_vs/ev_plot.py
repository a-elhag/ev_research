...
fig = np.empty(30, object)
ax = np.empty(30, object)

year = 0
for c, day in enumerate(range(0, 30)):
    fig[c] = plt.figure()
    ax[c] = plt.axes()
    ...
    ax[c].set_title(f"Year: {year}, Day: {day_name}, {date_num}")
    ax[c].legend()
    fig[c].savefig(f'../a_data/b_vs/plots/Y{year}-D{day}.png', dpi=300, bbox_inches='tight')
    plt.close(fig[c])
